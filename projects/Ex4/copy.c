...
/* Initial positions */
double Xold_up[DIM][N2]; // initial positions : 3D and 4 electrons
double Xnew_up[DIM][N2]; 
double Xold_down[DIM][N2]; 
double Xnew_down[DIM][N2]; 
double L = 3.0 * Ray;
printf("L: %lf\n",L);
double h = L/(DIMx -1.0);
...
for (int ii = 0; ii < DIM; ii++)
{
    ...
    {
        Xold_up[ii][jj] = Xnew_up[ii][jj] = 0.0;
        Xold_down[ii][jj] = Xnew_down[ii][jj] = 0.0;
    }
}
...
double r2_up[N2], r2_down[N2], r_up[N2], r_down[N2]; // radius squared and radius
...
/* ########################################## Initialization ############################################# */
    ...
    for (int ii = 0; ii < DIM; ii++)
    {
        for (int jj = 0; jj < N2; jj++)
        {
            Xold_up[ii][jj] = h * (rand()/(double)RAND_MAX - 1.0/2.0);
            Xold_down[ii][jj] = h * (rand()/(double)RAND_MAX - 1.0/2.0);
        }
    }
    ...
    {
        r2_up[ii] = Xold_up[0][ii] * Xold_up[0][ii] + Xold_up[1][ii] * Xold_up[1][ii] + Xold_up[2][ii] * Xold_up[2][ii]; 
        r_up[ii] = sqrt(r2_up[ii]);

        r2_down[ii] = Xold_down[0][ii] * Xold_down[0][ii] + Xold_down[1][ii] * Xold_down[1][ii] + Xold_down[2][ii] * Xold_down[2][ii]; 
        r_down[ii] = sqrt(r2_down[ii]);
    }
    ...
    // printf("\n");
    gradient_of_matrix(gradAup_x, eta[ee], r_up, Xold_up, 0); 
    // printf("gradAup_x\n");
    // print_mat_contents(gradAup_x);
    // printf("\n");
    gradient_of_matrix(gradAup_y, eta[ee], r_up, Xold_up 1); 
    // printf("gradAup_y\n");
    // print_mat_contents(gradAup_y);
    // printf("\n");
    gradient_of_matrix(gradAup_z, eta[ee], r_up, Xold, 2);
    // printf("gradAup_z\n");
    ...
    // printf("\n");
    initialize_mat_contents(Aup, eta[ee], r_up, Xold_up); // To do because invert_a_matrix reverts the matrix Aup
    ...
    // printf("detAup : %lf\n\n", detAup);
    initialize_mat_contents(Aup, eta[ee], r_up, Xold_up); // Re-initialize m


    /* Adown */
    // initialization_of_wf(Adown, gradAdown_x, gradAdown_y, gradAdown_z, lapAdown, GDratio_down, LDratio_down, detAdown, eta[ee], r, Xold);
    initialize_mat_contents(Adown, eta[ee], r_down, Xold_down); //Initialize matrix Aup
    gradient_of_matrix(gradAdown_x, eta[ee], r_down, Xold_down, 0); // Gradient of Adown - Adown_x
    gradient_of_matrix(gradAdown_y, eta[ee], r_down, Xold_down, 1); // Gradient of Adown - Adown_y
    gradient_of_matrix(gradAdown_z, eta[ee], r_down, Xold_down, 2); // Gradient of Adown - Adown_z
    laplacian_of_matrix(lapAdown, eta[ee], r_down, Xold_down); // Laplacian of Adown
    Adown_inv = invert_a_matrix(Adown); // Inverse of Adown
    initialize_mat_contents(Adown, eta[ee], r_down, Xold_down); // Re-initialize Adown
    ...
    {
        index = randomGenerator(0 , N); // take the evolving electron in a random way
        //printf("index: %d\n", index);
        ...
        for (int ii = 0; ii < DIM; ii++)
        {
            for (int jj = 0; jj < N2; jj++)
            {
                if (jj == index && index < 4)
                {
                    Xnew_up[ii][jj] = Xold_up[ii][jj] + h * (rand()/(double)RAND_MAX - 1.0/2.0);
                } else {

                    Xnew_up[ii][jj] = Xold_up[ii][jj];

                }
                
                if (jj == index && index > 3)
                {
                    Xnew_down[ii][jj] = Xold_down[ii][jj] + h * (rand()/(double)RAND_MAX - 1.0/2.0);
                } else {
                        
                     Xnew_down[ii][jj] = Xold_down[ii][jj];   
                }
...
/* Initialize matrix of positions of the 8 electrons - electrons can be in a cube of dimension L*L*L */
    for (int jj = 0; jj < N; jj++) // columns : run over electrons
    {
        for (int ii = 0; ii < DIM; ii++) // rows : run over components (x,y,z)
        {
            Xold[ii][jj] = randomGenerator_double(0.0,L);
            if (jj < 4)
            {
